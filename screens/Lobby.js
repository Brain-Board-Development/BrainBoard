 ...
 * Updated: Start Game navigates to BoardGameScreen
 */

import React, { useState, useEffect } from 'react';
import {
  View, Text, StyleSheet, TouchableOpacity, FlatList,
  ActivityIndicator, SafeAreaView, Modal, Image, Alert,
} from 'react-native';
...
export default function Lobby({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'gameSessions', sessionId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Non-host: auto-navigate when host starts the game
        if (!isHost && data.status === 'playing') {
          navigation.replace('BoardGameScreen', {
            sessionId,
            gameId,
            playerName: route.params.playerName || 'Player',
            isHost: false,
          });
        }
      } else {
  ...
  const handleStartGame = async () => {
    ...
    try {
      ...
      // Host navigates immediately; other players navigate via snapshot listener
      navigation.replace('BoardGameScreen', {
        sessionId,
        gameId,
  ...
  const renderPlayer = ({ item }) => (
    <View style={[styles.playerCard, hoveredButton === `player-${item.name}` && styles.playerCardHover]}>
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>Waiting for players to join...</Text>}
        />
      </View>
      ...
      {isHost && (
        <View style={styles.hostControls}>
          <TouchableOpacity
            style={[styles.lockButton, isLobbyLocked && styles.lockButtonActive, hoveredButton === 'lock' && styles.lockButtonHover]}
            onPress={toggleLobbyLock}
            onMouseEnter={() => setHoveredButton('lock')}
            onMouseLeave={() => setHoveredButton(null)}
          >
            <Text style={{ fontSize: 28 }}>{isLobbyLocked ? '🔒' : '🔓'}</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.startButton, !canStart && styles.startButtonDisabled, hoveredButton === 'start' && styles.startButtonHover]}
      ...
      <Modal visible={showCancelModal} transparent animationType="fade" onRequestClose={() => setShowCancelModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.confirmModal}>
            ...
            <Text style={styles.confirmModalText}>Are you sure you want to cancel and leave this lobby?</Text>
            <View style={styles.confirmModalButtons}>
              <TouchableOpacity style={styles.confirmModalCancel} onPress={() => setShowCancelModal(false)}>
...
const styles = StyleSheet.create({
  ...
  pinCard: { backgroundColor: '#1e1e1e', borderRadius: 16, padding: 24, alignItems: 'center', marginBottom: 30, borderWidth: 1, borderColor: '#333' },
  pinLabel: { fontSize: 18, color: '#aaa', marginBottom: 12 },
  pin: { fontSize: 56, fontWeight: 'bold', color: '#00c781', letterSpacing: 10 },
  playerCountContainer: { alignItems: 'center', marginBottom: 30 },
  playerCount: { color: '#00c781', fontSize: 20, fontWeight: '600' },
  playersList: { paddingBottom: 140 },
  playerRow: { justifyContent: 'space-between', marginBottom: 16 },
  playerCard: { flex: 1, marginHorizontal: 8, backgroundColor: '#1e1e1e', borderRadius: 16, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: '#333' },
  playerCardHover: { borderColor: '#00c781' },
  playerName: { color: '#fff', fontSize: 18, fontWeight: '600' },
  emptyText: { color: '#666', fontSize: 18, textAlign: 'center', marginTop: 60 },
  hostControls: { position: 'absolute', bottom: 100, left: 0, right: 0, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 20, paddingHorizontal: 30 },
  lockButton: { width: 70, height: 70, backgroundColor: '#1e1e1e', borderRadius: 16, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#333' },
  lockButtonActive: { backgroundColor: '#003322', borderColor: '#00c781' },
  lockButtonHover: { borderColor: '#00c781' },
  startButton: { backgroundColor: '#00c781', borderRadius: 16, paddingVertical: 18, paddingHorizontal: 40, alignItems: 'center', minWidth: 200 },
  startButtonHover: { backgroundColor: '#00e092' },
  startButtonDisabled: { backgroundColor: '#444', opacity: 0.6 },
  startButtonText: { color: '#000', fontSize: 20, fontWeight: 'bold' },
  cancelButton: { position: 'absolute', bottom: 40, left: 30, backgroundColor: '#c0392b', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 16 },
  cancelButtonHover: { backgroundColor: '#d32f2f' },
